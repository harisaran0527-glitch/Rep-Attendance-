/**
 * Native Canvas & Web Share API Module for CR Attendance Manager
...
export interface ShareStatusReportParams {
  ...
  subject: string;
  period: number;
  status: string; // 'Present', 'Absent', 'OD', 'Medical Leave', 'Long Absent', 'ALL'
...
export async function generateStatusReportImageCanvas(params: ShareStatusReportParams): Promise<File> {
  const { date, subject, period, status, studentList, fileName } = params;
  ...
  ctx.fillText('CR ATTENDANCE MANAGER', padding, 38);

  ctx.fillStyle = '#94a3b8';
  ctx.font = '500 13px system-ui, -apple-system, sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText('OFFICIAL ATTENDANCE REPORT', canvasWidth - padding, 38);
  ctx.textAlign = 'left';

  // 6. Report Metadata (Date, Subject, Period)
  const metaY = 85;
  ...
  ctx.font = '500 13px system-ui, -apple-system, sans-serif';
  ctx.fillText(`Subject: ${subject}  |  Period ${period}`, padding, metaY + 38);
  ...
  if (sLower.includes('present')) {
    ...
    badgeColor = '#1e40af';
    badgeBorder = '#bfdbfe';
  } else if (sLower.includes('ml') || sLower.includes('medical leave')) {
    badgeBg = '#f3e8ff';
    badgeColor = '#6b21a8';
    badgeBorder = '#e9d5ff';
  } else if (sLower.includes('la') || sLower.includes('long absent')) {
    badgeBg = '#f4f4f5';
    badgeColor = '#27272a';
    badgeBorder = '#e4e4e7';
  }
  ...
  ctx.textAlign = 'right';
  ctx.fillText('Official Read-Only Attendance Record', canvasWidth - padding, footerY + 15);
  ctx.textAlign = 'left';
