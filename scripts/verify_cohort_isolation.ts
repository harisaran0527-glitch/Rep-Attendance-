...
import { calculateAllStudentsAttendanceStats, getValidWorkingDates, invalidateCache } from '../src/lib/db-api';
...
async function main() {
  ...
  const testLateralDate = new Date('2026-08-17T00:00:00.000Z');
  const testRegularDate = new Date('2026-08-18T00:00:00.000Z');

  // Baseline
  invalidateCache();
  const baseRegDays = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'REGULAR')).length;
  const baseLatDays = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'LATERAL_ENTRY')).length;

  console.log(`Baseline Working Days -> REGULAR: ${baseRegDays}, LATERAL_ENTRY: ${baseLatDays}`);

  try {
    // Step A: Add a Lateral Entry attendance date on 2026-08-17 (unmarked past date)
    ...
    const regDaysAfterLatAdd = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'REGULAR')).length;
    const latDaysAfterLatAdd = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'LATERAL_ENTRY')).length;

    console.log(`\n[Test A] Added Lateral Entry Attendance on 2026-08-17:`);
    ...
    // Step B: Add a Regular attendance date on 2026-08-18 (unmarked past date)
    ...
    const regDaysAfterRegAdd = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'REGULAR')).length;
    const latDaysAfterRegAdd = (await getValidWorkingDates('2026-07-13', '2026-08-20', 'LATERAL_ENTRY')).length;

    console.log(`\n[Test B] Added Regular Attendance on 2026-08-18:`);
